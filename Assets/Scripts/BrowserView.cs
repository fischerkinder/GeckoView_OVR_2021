...
public class BrowserView : MonoBehaviour 
{
    ...
    public RawImage ClickPanel;
    public OVROverlay OverlayObject;

  
    private Texture2D _imageTexture2D;
    private static IntPtr _externalSurfacePtr = System.IntPtr.Zero;
    public GameObject TextureDestinationGameObject;
    ...
    protected private static AndroidJavaObject _activityContext = null;
    ...
    // Call this to add keys to GeckoView browser   
    ...
    private void OnClick()
    {
        ...
        positionInHD.x = 1920 - ((float)(ControllerRayInteractor.CollisionInfo.Value.Point.x - 0.8) * 1000 / 2 * -1);   // FixMe: Canvas size schould be taken from clickpanel
        positionInHD.y = 1080 - ((float)(ControllerRayInteractor.CollisionInfo.Value.Point.y - 1) * 1000 / 2);
        Debug.Log("PixelCoords: " + positionInHD);

        // Propagate ClickPanel coordinates to the geckoView wrapper (.aar plugin)
        if (_ajc != null)
        {
    ...
    public byte[] TakePngScreenShot(int width=0, int height=0, int quality=100)
    {
        ...
        Debug.Log("_TAKE GetSurfaceBytesBuffer " + Time.realtimeSinceStartup);
        AndroidJavaObject jo = _ajc.Call<AndroidJavaObject>("GetSurfaceBytesBuffer", new object[]{width, height, quality});  // <<<--- Takes ages
        Debug.Log("_TAKE Buffer " + Time.realtimeSinceStartup);
        AndroidJavaObject bufferObject = jo.Get<AndroidJavaObject>("Buffer");
        Debug.Log("_TAKE before Bufferconversion. " + Time.realtimeSinceStartup);
        byte[] bytes = AndroidJNIHelper.ConvertFromJNIArray<byte[]>(bufferObject.GetRawObject());
        Debug.Log("_TAKE after Bufferconversion. " + Time.realtimeSinceStartup);
        return bytes;
    }
...
#region Initialization
    ...
    private void Awake()
    {
        ...
        // This is a Panel used to determine where someone has clicked.
        // Used for "touch" input to the geckoview
        ClickPanel.GetComponent<Button>().onClick.AddListener(OnClick);
        _rawImageRect = ClickPanel.GetComponent<RectTransform>();
        ...
        //Test Start
         _exportTexture = new Texture2D(1920, 1080, DefaultFormat.LDR, TextureCreationFlags.None);
    ...
    private IEnumerator Start()
    {
            ...
            yield break;
#endif
        // Wait for surface (the former OVR?) to be available, by using coroutine wait style.
        while (_overlay.externalSurfaceObject == IntPtr.Zero || _ajc == null)
        {
            yield return null;
        }

        _imageTexture2D = new Texture2D(1920, 1080, TextureFormat.RGBA32, false);
        _imageTexture2D.filterMode = FilterMode.Point;
        _imageTexture2D.Apply();

        TextureDestinationGameObject.GetComponent<MeshRenderer>().material.mainTexture = _imageTexture2D;


        _externalSurfacePtr =  _imageTexture2D.GetNativeTexturePtr();// There is no external texture surfece currently !!!? We may not need one. we only want to transfer textures from existing surface..
        ...
        AndroidJNI.CallVoidMethod(pluginObject, surfaceMethodId, new jvalue[] { new jvalue { l = _overlay.externalSurfaceObject } });

        //AndroidJNI.CallVoidMethod(pluginObject, surfaceMethodId, new jvalue[] { new jvalue { l = _externalSurfacePtr } }); 
        //_imageTexture2D.UpdateExternalTexture(_externalSurfacePtr);

        //AndroidJNI.CallVoidMethod(pluginObject, surfaceMethodId,
        //    new jvalue[] { new jvalue { l = _imageTexture2D.GetNativeTexturePtr()    } });
    ...
    public void InitializeAndroidPlugin()
    {
        ...
        var tempAjc = new AndroidJavaClass("com.eyeflite.ian.geckoviewplugin.GeckoViewPLugin");
        ...
        //_ajc = tempAjc.CallStatic<AndroidJavaObject>("CreateInstance", new object[] { _surfaceWidth, _surfaceHeight, UserAgent.mobile.ToString("G") });
        
        

        UnityInterface androidPluginCallbacks = new UnityInterface {BrowserView = this};
...
//        Debug.Log("position in webview: " + positionInWebView);
    ...
    public void UpdateProgress(int progress)
    {
        ...
        Debug.Log("_TAKESNAP before take:" + Time.realtimeSinceStartup);
        byte[] rawImage = TakePngScreenShot(1920, 1080, 100);
        Debug.Log("_TAKESNAP before load:" + Time.realtimeSinceStartup);
        _exportTexture.LoadImage(rawImage);
        Debug.Log("_TAKESNAP after load:" + Time.realtimeSinceStartup);

        destinationCube.GetComponent<MeshRenderer>().material.mainTexture = _exportTexture;
